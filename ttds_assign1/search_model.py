...
def load_query_if_q_beginning(path):
    """
    In the lab2, we use this function to load queries;
    but in the assignment test, the structure changed, this function is not used in the final dataset

    :param path: queries path
    :return: queries data structure {query_id: query}
    """
    queries = OrderedDict()  # {query_id: query}
    with open(path, 'r') as f:
        for line in f:
            query = line.strip().split(' ', 1)
            query_id = query[0].strip('q:')
...
def load_query(path):
    """
    load queries both for boolean and ranked in the final dataset
...
def ranked_search(index, queries, doc_all_ids_set):
    """
    ...
    :return: the search results {query_id: [doc_id, score]}
    """
    n = len(doc_all_ids_set)  # number of documents in a collection
    search_results = OrderedDict()  # {query_id: [doc_id, score]}
    for query_id, query_content in queries.items():
        ...
        search_results[query_id] = scores_list
