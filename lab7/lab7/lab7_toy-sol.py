...
# the count for word fresh for the first sentence
i = word_vocab.index("fresh")
# print(train_word_features[0][i])
# the count of the POS tag ’NNS’ in the third training sentence
j = pos_vocab.index("NNS")


# print(train_pos_features[2][j])
...
def class_priors(training_y):
    ...
    log_probs = np.log(counts / sum(counts))  # broadcast
...
def feature_probabilities(training_x, training_y, alpha):
    ...
    for c in set(training_y):  # for each category
        ...
        c_data += alpha
        f_probs[c] = np.log(c_data / c_data.sum())
